...
MULTIPLE CLIPS (PLAYLIST)
-------------------------
The hero can rotate through several clips, crossfading between them with no
black flash. To add clips:

  1. Encode each clip the same way (same 1600x900 size, no audio, under ~2MB).
     Name them e.g. hero.mp4, water.mp4, aerial.mp4.
  2. Drop them in this folder.
  3. Open assets/js/app.js, find the PLAYLIST array inside initHeroVideo(),
     and add one line per clip:

        var PLAYLIST = [
          { webm: 'assets/videos/hero.webm', mp4: 'assets/videos/hero.mp4' },
          { mp4: 'assets/videos/water.mp4' },
          { mp4: 'assets/videos/aerial.mp4' }
        ];

     webm is optional per clip; mp4 is the one that plays everywhere.

The list loops forever: after the last clip it crossfades back to the first.
A single clip (the default) simply loops itself. Do NOT put a "loop" attribute
on the <video> tags in index.html — the JavaScript handles looping. With loop
on the tag, the clip restarts instead of advancing and the playlist never moves.

All clips should share the same dimensions, or you'll see a size jump between
them.
