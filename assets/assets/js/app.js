...
(function () {
  ...
  function initHeroVideo() {
    ...
    if (reduceMotion || saveData) { box.remove(); return; }
    ...
    // Single clip: just loop it on one layer, no crossfade machinery needed.
    if (PLAYLIST.length < 2) {
      var only = layers[0];
      only.loop = true;
      setClip(only, PLAYLIST[0]);
      var settled0 = false;
      only.addEventListener('canplay', function () {
        if (!settled0) { settled0 = true; box.dataset.ready = 'true'; }
      }, { once: true });
      only.addEventListener('error', function () { box.remove(); }, { once: true });
      tryPlay(only);
      setTimeout(function () { if (only.readyState < 3) box.remove(); }, 6000);
      return;
    }
    ...
    function setClip(video, clip) {
      video.innerHTML = '';
      if (clip.webm) video.appendChild(sourceEl(clip.webm, 'video/webm'));
      if (clip.mp4)  video.appendChild(sourceEl(clip.mp4,  'video/mp4'));
      video.load();
    }
    function sourceEl(src, type) {
    ...
    // Preload the next clip onto the hidden layer, then crossfade on 'ended'.
    function primeNext() {
      var hidden = layers[active ^ 1];
      var nextIdx = (idx + 1) % PLAYLIST.length;
      setClip(hidden, PLAYLIST[nextIdx]);
    }
    ...
    function advance() {
      ...
      // After the fade, rearm: pause the old layer and preload the following clip.
      setTimeout(function () {
        current.pause();
        primeNext();
      }, 1000);
    ...
    layers.forEach(function (v) {
      v.addEventListener('ended', function () {
        // Only the visible layer drives the sequence.
        if (v === layers[active]) advance();
      });
    });

    layers[0].addEventListener('canplay', function () {
      if (!ready) { ready = true; box.dataset.ready = 'true'; }
    }, { once: true });
    layers[0].addEventListener('error', function () {
      if (!ready) box.remove();
    }, { once: true });

    setClip(layers[0], PLAYLIST[0]);
    tryPlay(layers[0]);
    primeNext();

    setTimeout(function () { if (layers[0].readyState < 3) box.remove(); }, 6000);
  }
  ...
  function initSlideshow() {
    ...
    function start() { if (!timer) timer = setInterval(function () { show(i + 1); }, DELAY); }
  ...
  var Modal = (function () {
    ...
    function close(opts) {
      if (!openEl) return;
      var swapping = opts && opts.swap;
      openEl.classList.remove('is-open');
      openEl.setAttribute('aria-hidden', 'true');
      // During a modal-to-modal swap, keep the scroll lock and don't bounce
      // focus back to the trigger — the next modal takes over immediately.
      if (!swapping) {
  ...
  function initModalTriggers() {
    ...
    function initCarousel(root) {
      ...
      function start() { if (!carTimer) carTimer = setInterval(function () { show(i + 1); }, 4500); }
    ...
    if (detailQuote) {
      detailQuote.addEventListener('click', function () {
        ...
        // Swap: close without releasing the scroll lock or bouncing focus,
        // then open the next dialog once the fade-out has finished.
        Modal.close({ swap: true });
        setTimeout(function () {
          openQuoteModal(isService ? 'service' : 'product', label);
        }, 300);
      });
    }
  ...
  function placePanel(root, panel) {
    ...
    var GAP = 12;              // breathing room from the pane edges
    var r = root.getBoundingClientRect();

    // Constrain to the scrolling pane if inside one, else the viewport.
    var pane = root.closest('.qform__body') || root.closest('.modal__box');
    var paneTop = pane ? Math.max(pane.getBoundingClientRect().top, 0) : 0;
    var paneBottom = pane
      ? Math.min(pane.getBoundingClientRect().bottom, window.innerHeight)
      : window.innerHeight;

    // The panel floats ~.45rem off the trigger; include that plus the edge gap
    // so the panel never runs past the pane. Slightly generous to be safe.
    var OFFSET = 16;
    var roomBelow = paneBottom - r.bottom - OFFSET - GAP;
    var roomAbove = r.top - paneTop - OFFSET - GAP;

    // Natural panel height (now that overrides are cleared and layout settled).
    var natural = panel.scrollHeight;

    // Prefer opening downward; flip up only when there's more room above
    // AND the panel doesn't already fit below.
    var wantUp = (natural > roomBelow) && (roomAbove > roomBelow);
    if (wantUp) root.classList.add('is-up');

    // True space on the chosen side. If it's genuinely tiny, flip to whichever
    // side has more room so we never cap to an unusable sliver.
    var chosen = wantUp ? roomAbove : roomBelow;
    if (chosen < 140 && Math.max(roomAbove, roomBelow) > chosen) {
      wantUp = roomAbove > roomBelow;
      root.classList.toggle('is-up', wantUp);
      chosen = wantUp ? roomAbove : roomBelow;
    }
    var avail = Math.max(120, Math.floor(chosen));

    // If the panel is taller than the space on the chosen side, cap it.
    if (natural > avail) {
      if (panel.classList.contains('pick__panel--date')) {
        panel.style.maxHeight = avail + 'px';
        panel.style.overflowY = 'auto';
      } else if (list) {
        var chrome = panel.offsetHeight - list.offsetHeight;
        list.style.maxHeight = Math.max(88, avail - chrome) + 'px';
      } else {
        panel.style.maxHeight = avail + 'px';
        panel.style.overflowY = 'auto';
      }
    ...
    // Final safety: measure where the panel actually lands and, if it still
    // crosses a pane edge, clamp it hard. Runs next frame so the CSS above
    // has been applied. Guards against offset/padding rounding on any device.
    requestAnimationFrame(function () {
      if (!root.classList.contains('is-open')) return;
      var pr = panel.getBoundingClientRect();
      var over = 0;
      if (root.classList.contains('is-up')) {
        over = paneTop - pr.top;                 // clipped at the top
      } else {
        over = pr.bottom - paneBottom;           // runs past the bottom
      }
      if (over > 0) {
        var target = pr.height - over - 2;
        if (list) {
          var extra = panel.getBoundingClientRect().height - list.getBoundingClientRect().height;
          list.style.maxHeight = Math.max(80, target - extra) + 'px';
        } else {
          panel.style.maxHeight = Math.max(120, target) + 'px';
          panel.style.overflowY = 'auto';
        }
      }
    });
