...
The homepage hero plays a looping background video if you provide one. Put
`hero.mp4` and `hero.webm` in `assets/videos/`. Full encoding instructions are
in `assets/videos/README.txt`.

Without those files the hero shows `assets/images/hero-bg.jpg` instead — the
site works fine either way. Keep the video under about 4MB; anything larger
noticeably hurts load time on mobile connections.
